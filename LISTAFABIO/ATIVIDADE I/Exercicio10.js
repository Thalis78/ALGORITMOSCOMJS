import { get_number, get_number_min, get_positive_number, print } from './apelidos.js'


function main(){
  const qtd_containers = get_number('Qtd Containers: ', 0)
  ...
  while (bilhete !== 0){
    ...
    const bagagens = get_number('Qtd de Bagagens: ', 0)
...
function computar_carga(quantidade){
  ...
  while (qtd_containers_lidos < quantidade){
    ...
    const peso = get_number('Peso Container: ')
