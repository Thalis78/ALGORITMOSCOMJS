...
function main() {
    ...
    let quantLetras = Number(input("QUANTIDADE DE LETRAS (MIN:3, MAX 9): "));
    quantLetras < 10 ? quantLetras : main()

    gerarItens(RGB, R, Math.floor(quantLetras / 3)), gerarItens(RGB, G, quantLetras % 3 === 1 ? parseInt(quantLetras / 3) + 1 : Math.round(quantLetras / 3)), gerarItens(RGB, B, Math.round(quantLetras / 3))
    ...
    while (count != quantLetras || jogadores.length !== 2) {
        count = 0
        ...
        if (count === quantLetras && jogadores.length < 1) {
